    ...
    if (obj != null && typeof obj === 'object' &&
        ...
        utils.isVariable(key)
            ...
            for (var x in obj) {
                ...
                core.setValue(val+(rand)+'.'+this.level, obj[x], goalList, env, rules,  accum); 
                core.setValue(key+(rand)+'.'+this.level, x, goalList, env, rules,  accum);
...
/*exports.bind = function bind (list,  goalList, env, rules,  accum) {
    var result = null;
    if (list.obj.length === 2) {  
        var arg1 = list.obj[0];
        var arg2 = list.obj[1];
        var dummyVar = '$x' + parseInt(Math.random(1000)*1000);
        var obj1 = new RenamedVarsObject ({bind : [dummyVar, dummyVar]}, this.level);
        var obj2 = new RenamedVarsObject ({bind : [arg1, arg2]}, list.level);
        result = core.unify.apply(this, [obj1, obj2, goalList, core.envWrap(env), rules, accum]);
        if (result != null) { 
            this.level++;
            result = core.qeval.apply(this, [goalList, core.envClone(result, true), rules,  accum, true]);
        }
...
exports.$bind = null; // placeholder to define $bind builtin. The actual implementation is via util-rules.js 
