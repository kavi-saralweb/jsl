...
function getUniFunc (goalList, env, rules, accum) { 
    return  function(x,y) {
        ...
        if (utils.isObject(x) && utils.isEmptyObject(y)) { 
            ...
            result = exports.setValue(x, y, false, goalList, env, rules,  accum);
        }
        else if (utils.isVariable(y)) {
            result = exports.setValue(y, x, true, goalList, env, rules,  accum);
        }
...
exports.setValue = function(variable, value, flip, goalList, env, rules,  accum) {
    var result = null;
    var v;
    //console.log('setValue: ', variable, value, env[variable] != null);
    if (v = env[variable]) { // binding already exists
        if (flip) {
            result = exports.unify(value, v, goalList, env, rules,  accum);
        }
        else {
